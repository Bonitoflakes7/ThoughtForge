...
function AdminUserPanel() {
  ...
  return <section className="admin-panel"><div className="desk-heading"><div><p className="eyebrow">Admin control</p><h1>People & roles<span>.</span></h1><p className="muted">Shape who can care for the room.</p></div></div>{error && <p className="form-error page-error">{error}</p>}<div className="admin-user-list">{users.map((item) => <div className={`admin-user-row ${item.is_active ? "account-active" : "account-inactive"}`} key={item.id}><div className="admin-user-identity"><Initials name={item.display_name} /><div><strong>{item.display_name}</strong><small>@{item.username} · {item.email}</small></div></div><span className={`account-status ${item.is_active ? "active" : "inactive"}`}>{item.is_active ? "Active" : "Deactivated"}</span><select value={item.role} onChange={(e) => changeRole(item.id, e.target.value as User["role"])}><option value="user">User</option><option value="moderator">Moderator</option><option value="admin">Admin</option></select><button className="account-toggle" onClick={() => toggleStatus(item)}>{item.is_active ? "Deactivate" : "Activate"}</button></div>)}</div></section>;
}
